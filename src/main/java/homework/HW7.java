...
30. Прочитать ссылку про оператор switch в документации, посмотреть видео, и переписать проверки дат из кода,
который выставляла Гаяна в субботу, 8 октября (ДЗ 6, последняя задача), с помощью оператора switch.
В документации Oracle есть практически готовое решение + Гаяна уже все прописала, только надо “допилить” код
под наши нужды.
